...
namespace ParallelAssemblyLineNET.Tests
{
    public class Tests
    {
        ...
        [Test]
        public void V2SimpleButLongerFunctionalityLongRunningWithRandomDelayParallelOutputTest()
        {
            int countOfItemsToTest = 2000;
            ...
            ,new ParallelAssemblyLineOptions() { threadCreationOptions = TaskCreationOptions.LongRunning,outputThreads=8 }, (status) => {
                TestContext.WriteLine($"inbuf: {status.InputBufferSize},outbuf: {status.OutputBufferSize},finished: {status.DigestedItems},active: {status.ProcessingItems}\n");
            });
